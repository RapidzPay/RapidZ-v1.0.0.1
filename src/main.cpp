...
unsigned int nStakeTargetSpacing = 60 * 5;

int64 nChainStartTime = 1522303200;
...
int64 GetProofOfWorkReward(int nHeight, int64 nFees, const CBlockIndex* pindex)
{
     ...
     if (nPoWHeight == 1)
       nSubsidy = CIRCULATION_MONEY; //Premin: 10M
     else if (nPoWHeight < 100000002)
       nSubsidy = 0.1 * COIN;
...
int64 GetProofOfStakeReward(int64 nCoinAge, unsigned int nBits, unsigned int nTime, int nHeight, const CBlockIndex* pindex)
{
    int nPoSHeight = 0;
    int64_t nSubsidy = 0 * COIN;
    ...
    if(nHeight > 1)
      nPoSHeight = GetPosHeight(pindex);
    else
      nPoSHeight = nHeight;

    if (nPoSHeight <= 52704)
      nSubsidy = nCoinAge * MAX_CLOAK_PROOF_OF_STAKE * 50 / 365;
    else if (nPoSHeight <= 105120)
      nSubsidy = nCoinAge * MAX_CLOAK_PROOF_OF_STAKE * 40 / 365;
    else if (nPoSHeight <= 157824)
      nSubsidy = nCoinAge * MAX_CLOAK_PROOF_OF_STAKE * 30 / 365;
    else
      nSubsidy = nCoinAge * MAX_CLOAK_PROOF_OF_STAKE * 20 / 365;
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake) 
{
    ...
    int64 nStakeTargetSpacing = GetStakeTargetSpacing(pindexLast->nHeight);
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    if (!IsCoinBase())
    {
        ...
        if (IsCoinStake())
        {
            ...
            if (nStakeReward > GetProofOfStakeReward(nCoinAge, pindexBlock->nBits, nTime, pindexBlock->nHeight, pindexBlock->pprev) - GetMinFee() + MIN_TX_FEE)
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "March 29, 2018. The RapidZ release.";
        CTransaction txNew;
      ...
      if (true  && (block.GetHash() != hashGenesisBlock))
        ...
        assert(block.hashMerkleRoot == uint256("0xe7824592f95c0372db8fc4ea2d30c94faa9994b0ce95b2ca4579cb7b0d0f0cb4"));
...
unsigned char pchMessageStart[4] = { 0xda, 0x82, 0xfe, 0xdb };
